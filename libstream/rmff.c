...
static void
rmff_dump_fileheader(rmff_fileheader_t *fileheader, char *buffer)
{
...
static void
rmff_dump_prop(rmff_prop_t *prop, char *buffer)
{
  if (!prop) return;
...
static void
rmff_dump_mdpr(rmff_mdpr_t *mdpr, char *buffer)
{
  int s1, s2, s3;
...
static void
rmff_dump_cont(rmff_cont_t *cont, char *buffer)
{
  int p;
...
static void
rmff_dump_dataheader(rmff_data_t *data, char *buffer)
{
  if (!data) return;
...
rmff_dump_header(rmff_header_t *h, char *buffer, int maxs)
{
  ...
  if (stream)
  {
    while(*stream)
    {
      if ((written + (int)(*stream)->size) >= maxs)
        return -1;
  ...
  if ((written + sizeof(rmff_data_t)) >= maxs)
    return -1;
  rmff_dump_dataheader(h->data, &buffer[written]);
  written += sizeof(rmff_data_t);
...
static rmff_fileheader_t *
rmff_scan_fileheader(const char *data)
{
  rmff_fileheader_t *fileheader = 	\
	   (rmff_fileheader_t *)malloc(sizeof(rmff_fileheader_t));
...
{
  rmff_prop_t *prop = (rmff_prop_t *)malloc(sizeof(rmff_prop_t));
...
{
  rmff_mdpr_t *mdpr = (rmff_mdpr_t *)malloc(sizeof(rmff_mdpr_t));
  ...
  mdpr->stream_name = (char *)malloc(sizeof(char)*(mdpr->stream_name_size+1));
  ...
  mdpr->mime_type = (char *)malloc(sizeof(char)*(mdpr->mime_type_size+1));
  ...
  mdpr->type_specific_data = (char *)malloc(sizeof(char)*(mdpr->type_specific_len));
...
{
  rmff_cont_t *cont = (rmff_cont_t *)malloc(sizeof(rmff_cont_t));
  int pos;
  ...
  cont->copyright = (char *)malloc(sizeof(char)*(cont->copyright_len+1));
  ...
  cont->comment = (char *)malloc(sizeof(char)*(cont->comment_len+1));
...
{
  rmff_data_t *dh = (rmff_data_t *)malloc(sizeof(rmff_data_t));
...
{
  rmff_header_t *header = (rmff_header_t *)malloc(sizeof(rmff_header_t));
  ...
  header->streams = (rmff_mdpr_t **)	\
	   malloc(sizeof(rmff_mdpr_t*)*(header->fileheader->num_headers));
  for (i=0; i < (int)header->fileheader->num_headers; i++)
    header->streams[i]=NULL;
  
  for (i=1; i < (int)header->fileheader->num_headers; i++)
  {
    chunk_type = BE_32(ptr);
...
rmff_scan_header_stream(int fd)
{
  ...
  do {
    xbuffer_ensure_size(buf, index+8);
    read(fd, buf+index, 8);
    ...
    switch (chunk_type) {
      ...
      case RMF_TAG:
      case PROP_TAG:
        xbuffer_ensure_size(buf, index+chunk_size-8);
...
/**
 * scans a data packet header. Notice, that this function does not <BR>
 * allocate the header struct itself.
 */
extern int
...
rmff_scan_pheader_stream(int fd)
{
	int ret = -EINVAL;
	int size = 0;
	rmff_pheader_t *h = NULL;
	
	char datas[sizeof(rmff_pheader_t) + 4] = { 0 };
	
	h = rmff_new_pheader(0, 0, 0);
	if (NULL == h)
	{
		ret = -ENOMEM;
		goto errout;
	}
	ret = read(fd, datas, sizeof(rmff_pheader_t));
	if (ret < sizeof(rmff_pheader_t))
	{
		rmff_free_pheader(h);
		ret = -ENODATA;
		if (errno > 0)
	...
	rmff_scan_pheader(h, size, datas);
	ret = size;
      ...
      const char *type_specific_data)
{
  ...
  mdpr->type_specific_data = (char *)malloc(sizeof(char)*type_specific_len);
  ...
  mdpr->max_total_num = 0;
  mdpr->max_total_size = 0;
  
  return mdpr;
}

extern rmff_cont_t *
rmff_new_cont(const char *title, const char *author,
...
extern rmff_data_t *
rmff_new_dataheader(uint32_t num_packets, uint32_t next_data_header)
{
  rmff_data_t *data = (rmff_data_t *)malloc(sizeof(rmff_data_t));
...
{
  int size = sizeof(rmff_pheader_t) + data_size;
  rmff_pheader_t *pheader = (rmff_pheader_t *)malloc(size);
  
  pheader->object_version = 0;
  pheader->length = data_size;
  pheader->stream_number = stream_number;
...
rmff_fix_header(rmff_header_t *h)
{
  ...
  if (!h->data) {
    ...
    h->data = (rmff_data_t *)malloc(sizeof(rmff_data_t));
  ...
  if (!h->fileheader) {
    ...
    h->fileheader = (rmff_fileheader_t *)malloc(sizeof(rmff_fileheader_t));
  ...
  if((int)h->fileheader->num_headers != num_headers) {
    lprintf("rmff_fix_header: setting num_headers from %i to %i\n", h->fileheader->num_headers, num_headers); 

    h->fileheader->num_headers = (uint32_t)num_headers;
  }

  if (h->prop) {
    if ((int)h->prop->data_offset != header_size) {
...
rmff_output_header(rmff_header_t *h, int maxs, char *buffer)
{
	...
	if (h->fileheader != NULL)
		size += (int)(h->fileheader->size);
	
	if (h->prop != NULL)
		size += (int)(h->prop->size);
	if (h->cont != NULL)
		size += (int)(h->cont->size);
	...
	if (h->streams != NULL)
	{
		...
		while (*streams)
		{
			size += (int)((*streams)->size);
			streams++;
		}
	}
	
	if (h->data != NULL)
		size += (int)sizeof(rmff_data_t);
	if (size > maxs)
	{
		ret = -ENOSPC;
		goto errout;
	...
	ret = rmff_dump_header(h, buffer, maxs);
...
{
	int size = 0;
	rmff_sindex_t *n = NULL;
	
	size = rmff_resize_sindex(&n, count);
	if (NULL == n)
		return NULL;
	return n;
}
...
rmff_resize_sindex(rmff_sindex_t **s, int maxitem)
{
	...
	size = sizeof(rmff_sindex_t);
	if (maxitem > 0)
		size += maxitem * RMFF_SITEM_SIZE;
	if (m != NULL)
		count = m->entry_count;
	n = (rmff_sindex_t *)realloc(m, size);
	if (NULL == n)
		return -ENOMEM;
...
{
	int size = 0;
	rmff_stable_t *n = NULL;
	
	n = (rmff_stable_t *)malloc(sizeof(rmff_stable_t));
	if (NULL == n)
		return n;
	memset(n, 0, sizeof(rmff_stable_t));
	n->roffs = -1;
	n->scount = count;
	...
	size = ((count + 1) * sizeof(rmff_sindex_t *));
	n->sindex = (rmff_sindex_t **)malloc(size);
	if (NULL == n->sindex)
	{
		rmff_free_stable(n);
		return NULL;
...
rmff_dump_sindex(rmff_sindex_t *s, int maxsize, char *data)
{
	...
	int size = 0;
	int count = 0;
	rmff_sindex_t *w = NULL;
	
	size = sizeof(rmff_sindex_t);
	if (s->entry_count > 0)
	{
		count = s->entry_count;
		size = RMFF_SINDEX_SIZE(count);
	}
	if (NULL == data)
		return size;
	...
	memcpy(data, s, size);
	w = (rmff_sindex_t *)data;
	
	w->object_id = BE_32(&s->object_id);
	w->size = BE_32(&s->size);
	w->object_version = BE_16(&s->object_version);
	
	w->entry_count = BE_32(&s->entry_count);
	w->stream_number = BE_16(&s->stream_number);
	w->next_offset = BE_32(&s->next_offset);
	
	for (i = 0; i < count; i++)
	{
		w->entries[i].version = BE_16(&s->entries[i].version);
		w->entries[i].stamp = BE_32(&s->entries[i].stamp);
...
{
	s->object_id = INDX_TAG;
	s->size = sizeof(rmff_sindex_t);
	s->object_version = 0;
...
rmff_dump_stable(int fd, int offset, rmff_stable_t *t)
{
	...
	for (i = 0; i < t->scount; i++)
	{
		if (NULL == t->sindex[i])
			continue;
		size = RMFF_SINDEX_SIZE(t->sindex[i]->entry_count);
	...
	for (i = 0; i < t->scount; i++)
	{
		...
		ret = rmff_dump_sindex(t->sindex[i], smaxs, (char *)strdata);
		if (ret < t->sindex[i]->entry_count)
		{
			size = RMFF_SINDEX_SIZE(ret);
			if ((i + 1) >= t->scount)
				next = 0;
			else
				next += size;
...
rmff_resize_stable(int streamid, int maxsize, rmff_stable_t *t)
{
	...
	ret = rmff_resize_sindex(&t->sindex[streamid], maxsize);
	if (ret < 0)
		goto errout;
	 ...
	 uint32_t package, uint32_t pcount, rmff_pheader_t *h)
{
	...
	else
	{
		...
		ret = rmff_resize_stable(streamid, maxsize, t);
		if (ret < 0)
			goto errout;
		if (count <= 0)
		{
	...
	t->sindex[streamid]->entries[index].version = h->object_version;
	t->sindex[streamid]->entries[index].stamp = h->timestamp;
	t->sindex[streamid]->entries[index].offset = offset;
	t->sindex[streamid]->entries[index].package = package;
	
/**	fprintf(stderr, "%s stamp: %4u.%03u, offset: %8u, size: %4u, seq: %4u\n",
		 (streamid > 0) ? "video" : "audio",
		 h->timestamp/1000, h->timestamp%1000, offset, h->length, package);
 */
