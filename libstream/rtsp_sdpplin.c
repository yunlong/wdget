...
static char *
b64_decode(const char *in, char *out, int *size)
{
  uint8_t dtable[256];              /* Encode / decode table */
  int i,j,k;
...
static int filter(const char *in, const char *filter, char **out) {
  ...
  if (!strncmp(in,filter,flen))
  {
    ...
    if(in[len-1]==13) len--;
    if(in[len-1]=='"') len--;
    xbuffer_copyin(*out, 0, in+flen, len-flen+1);
    (*out)[len-flen]=0;
...
static sdpplin_stream_t *
sdpplin_parse_stream(char **data)
{
  sdpplin_stream_t *desc = 	\
	   (sdpplin_stream_t *)xutil_malloc(sizeof(sdpplin_stream_t));
  ...
  *data=nl(*data);
  ...
  while (*data && **data && *data[0]!='m') {
	...
	if(filter(*data,"a=AvgBitRate:integer;",&buf)) {
	  desc->avg_bit_rate=atoi(buf);
	  handled=1;
	  *data=nl(*data);
	}
	if(filter(*data,"a=AvgPacketSize:integer;",&buf)) {
	  desc->avg_packet_size=atoi(buf);
	  handled=1;
	  *data=nl(*data);
	}
    ...
    if(filter(*data,"a=ActualPreroll:integer;",&buf)) {
      desc->preroll=atoi(buf);
      handled=1;
      *data=nl(*data);
    }
    ...
    if(filter(*data,"a=OpaqueData:buffer;",&buf)) {
      ...
      desc->mlti_data = (char *)malloc(sizeof(char)*desc->mlti_data_size);
...
#ifdef LOG
      int len=strchr(*data,'\n')-(*data);
      xbuffer_copyin(buf, 0, *data, len+1);
      buf[len]=0;
...
sdpplin_t *sdpplin_parse(char *data) {
  ...
  sdpplin_t        *desc = (sdpplin_t *)xutil_malloc(sizeof(sdpplin_t));
  ...
  while (data && *data) {
    ...
    if(filter(data,"a=StreamCount:integer;",&buf)) {
      ...
      desc->stream = 	\
		   (sdpplin_stream_t **)malloc(sizeof(sdpplin_stream_t*)*desc->stream_count);
      handled=1;
      data=nl(data);
    }
