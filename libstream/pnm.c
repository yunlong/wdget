...
/**
 * pnm_get_chunk gets a chunk from stream <BR>
 * and returns number of bytes read, chunk_type, data and <BR>
 * a flag indicating whether the server want a response.
 *
 * following chunk format is expected (all in big endian!):
 *  <li> uint32_t chunk_type  |
 *  <li> uint32_t chunk_size  | <- preamble
 *  <li> uint8_t data[chunk_size-PREAMBLE_SIZE]
 *
	 ...
	 char *data, int *need_response)
{
  ...
  xio_tcp_read (p->s, data, CHECKSUM_SIZE);
  if (data[0] == 0x72)
    xio_tcp_read (p->s, data, PREAMBLE_SIZE);
  else
    xio_tcp_read (p->s, data+CHECKSUM_SIZE, PREAMBLE_SIZE-CHECKSUM_SIZE);
  ...
  *chunk_type = be2me_32(*((uint32_t *)data));
  ...
  switch (*chunk_type) {
	 ...
	 */
        ...
        if( max < 2 )
          return (unsigned int)-1;
        xio_tcp_read (p->s, ptr, 2);
        max -= 2;
        
	if (*ptr == 'X') /* checking for server message */
	{
	  xprintf(XVERBOSITY_DEBUG, "input_pnm: got a message from server:\n");
          if( max < 1 )
            return (unsigned int)-1;
	  xio_tcp_read (p->s, ptr+2, 1);
          max -= 1;
	  ...
	  /* message itself */
          if( max < n )
            return (unsigned int)-1;
	  xio_tcp_read (p->s, ptr+3, n);
          max -= n;
          if( max < 1 )
            return (unsigned int)-1;
	  ptr[3+n]=0;
	  xprintf(XVERBOSITY_DEBUG, "%s\n", ptr+3);
	  return (unsigned int)-1;
	}
	...
	if (*ptr == 'F') /* checking for server error */
	{
	  ...
	  xprintf(XVERBOSITY_DEBUG, "input_pnm: server error.\n");
	  return (unsigned int)-1;
	}
    ...
    case MDPR_TAG:
    case CONT_TAG:
      if (chunk_size > (int)max || chunk_size < PREAMBLE_SIZE) {
        xprintf(XVERBOSITY_DEBUG, "error: max chunk size exeeded (max was 0x%04x)\n", max);
#ifdef LOG
	/* reading some bytes for debugging */
        n=xio_tcp_read (p->s, &data[PREAMBLE_SIZE], 0x100 - PREAMBLE_SIZE);
        xine_hexdump(data,n+PREAMBLE_SIZE);
#endif
        return (unsigned int)-1;
      }
      xio_tcp_read (p->s, &data[PREAMBLE_SIZE], chunk_size-PREAMBLE_SIZE);
      break;
    default:
      *chunk_type = 0;
...
/**
 * writes a chunk to a buffer, returns number of bytes written <BR>
 *  <li> chunk format (big endian):
 *  <li> uint16_t chunk_id
 *  <li> uint16_t length
 *  <li> uint8_t data[length]
 */
static int
pnm_write_chunk(uint16_t chunk_id, uint16_t length, 
    const void *chunk, char *data)
{
 ...
 */
static void
pnm_send_request(pnm_t *p, uint32_t bandwidth)
{
  uint16_t i16;
  ...
  c+=pnm_write_chunk(PNA_CLIENT_CHALLANGE,strlen((char *)pnm_challenge),
  ...
  c+=pnm_write_chunk(PNA_TIMESTAMP,strlen((char *)pnm_timestamp),
  ...
  c+=pnm_write_chunk(PNA_GUID,strlen((char *)pnm_guid),
          pnm_guid,&p->buffer[c]);
  ...
  i16=be2me_16(strlen((char *)client_string)-1); /* dont know why do we have -1 here */
  memcpy(&p->buffer[c+1],&i16,2);
  memcpy(&p->buffer[c+3],client_string,strlen((char *)client_string)+1);
  c=c+3+strlen((char *)client_string)+1;
  ...
  xio_tcp_write(p->s, p->buffer, c+2);
}
 ...
 */
static void
pnm_send_response(pnm_t *p, const char *response)
{
  ...
  xio_tcp_write(p->s, p->buffer, size+3);

}

/**
 * get headers and challenge and fix headers <BR>
 *  <li> write headers to p->header
 *  <li> write challenge to p->buffer
 * 
 * @return 0 on error.  != 0 on success
 */
static int
pnm_get_headers(pnm_t *p, int *need_response)
{
  uint32_t chunk_type;
...
/*  rmff_header_t *h; */
  ...
  *need_response=0;
  ...
  while(1) {
    ...
    {
      xprintf(XVERBOSITY_DEBUG, "input_pnm: header buffer overflow. exiting\n");
      return 0;
    }
  ...
  if (!prop_hdr) {
    xprintf(XVERBOSITY_DEBUG, "input_pnm: error while parsing headers.\n");
    return 0;
  }
  ...
  xio_tcp_read (p->s, &p->buffer[PREAMBLE_SIZE], 64);
...
pnm_get_stream_chunk(pnm_t *p)
{
  ...
  if (p->buffer[0] == 'X')
  {
    ...
    xio_tcp_read (p->s, &p->buffer[8], size-5);
    p->buffer[size+3]=0;
  ...
  {
    xprintf(XVERBOSITY_DEBUG, "input_pnm: bad boundaries\n");
#ifdef LOG
  ...
  n = xio_tcp_read (p->s, &p->recv[5], fof1-5);
  if (n<((int)fof1-5)) return 0;
...
pnm_t *pnm_connect(const char *mrl)
{
  ...
  p = (pnm_t *)xutil_malloc(sizeof(pnm_t));
...
//  p->stream = stream;
  ...
  p->host = (char *)malloc(sizeof(char)*hostend+1);
  strncpy(p->host, mrl_ptr, hostend);
  p->host[hostend]=0;

  if (pathbegin < (int)strlen(mrl_ptr))
    p->path=strdup(mrl_ptr+pathbegin+1);
  if (colon != slash) {
  ...
  fd = xio_tcp_connect (p->host, p->port);

  if (fd == -1) {
    xprintf (XVERBOSITY_LOG, _("input_pnm: failed to connect '%s'\n"), p->host);
    free(p->path);
    free(p->host);
    free(p->url);
    free(p);
  ...
  if (!pnm_get_headers(p, &need_response)) {
    xprintf (XVERBOSITY_LOG, _("input_pnm: failed to set up stream\n"));
    free(p->path);
    free(p->host);
    free(p->url);
    free(p);
  ...
  if (need_response)
    pnm_send_response(p, (const char *)pnm_response);
  p->ts_last[0]=0;
  p->ts_last[1]=0;
...
pnm_read (pnm_t *session, char *data, int len)
{
  ...
  while (to_copy > fill) {
    ...
    if (!pnm_get_stream_chunk (session)) {
  ...
  xutil_fast_memcpy(dest, source, to_copy);
  session->recv_read += to_copy;
...
pnm_peek_header (pnm_t *session, char *data, int maxsize)
{
  ...
  memcpy(data, session->header, len);
  return len;
}
