...
asmrp_new ()
{
  asmrp_t *p = (asmrp_t *)malloc (sizeof (asmrp_t));
...
static void
asmrp_init (asmrp_t *p, const char *str)
{
  p->buf = strdup (str);
  p->pos = 0;
...
static int
asmrp_find_id (asmrp_t *p, char *s)
{
  int i;
...
static int
asmrp_set_id (asmrp_t *p, char *s, int v)
{
  int i;
...
static int
asmrp_condition (asmrp_t *p) ;

static int
asmrp_operand (asmrp_t *p)
{
  int i, ret;
  ...
  case ASMRP_SYM_LPAREN:
    ...
    if (p->sym != ASMRP_SYM_RPAREN)
    {
      ...
      x_assert(ASMRP_SYM_RPAREN == p->sym);
    }
...
static int
asmrp_comp_expression (asmrp_t *p)
{
...
asmrp_assignment (asmrp_t *p)
{
  ...
  {
    printf ("error: = expected\n");
	x_assert(ASMRP_SYM_EQUALS == p->sym);
  }
  asmrp_get_sym (p);

  if ((p->sym != ASMRP_SYM_NUM) && 
	  (p->sym != ASMRP_SYM_STRING) && (p->sym != ASMRP_SYM_ID))
  {
...
asmrp_rule (asmrp_t *p)
{
  ...
  if (p->sym != ASMRP_SYM_SEMICOLON)
  {
    ...
    x_assert(ASMRP_SYM_SEMICOLON == p->sym);
  }
...
static int
asmrp_eval (asmrp_t *p, int *matches)
{
